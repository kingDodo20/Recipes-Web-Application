...
app.secret_key = secrets.token_hex(16) #secret key for the session
...
def login():
    # Check cookies for Remember Me if the form is not submitted
    email_cookie = request.cookies.get('email')
    password_cookie = request.cookies.get('password')

    if email_cookie and password_cookie:
        # Attempt auto-login with cookies (you should hash the password for secure login)
    ...
    if request.method == "POST":
        ...
        if account_login(email, password):  
            ...
            # Set session cookie to expire in 7 days if "Remember Me" was selected
            remember_me = request.form.get('remember_me')
            if remember_me:
                resp = redirect(url_for('homepage'))
                resp.set_cookie('email', email, max_age=7*24*60*60)  # Cookie expires in 7 days
                resp.set_cookie('password', password, max_age=7*24*60*60)  # Cookie expires in 7 days
                return resp
            return redirect(url_for('homepage'))
        else:
...
def recipesPage():
    if "user_id" in session:
        data = get_recipes_data()  # Fetch all recipes from the database
        recipes = {
            row[1]: {
                "title": row[1],
                "description": row[2],
                "image_name": row[3],  # Ensure this field is included
        ...
        if request.method == 'POST':
            ...
            if image:
                try:
                    ...
                    print("Image path:", image_path)  # Debug: Print image path
...
def profile():
    ...
    if request.method == "POST":
        ...
        # Only check email update restrictions for the current user
        if new_email and new_email != current_email:
            ...
            # Update session after a successful email update
